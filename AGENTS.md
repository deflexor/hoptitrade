...
## Haskell constraint-evasion review

When reviewing a Haskell diff, PR, or Haskell you just wrote, read and
follow `.cursor/skills/haskell-constraint-evasion/SKILL.md`. Start from
the functions that changed and how they use the types involved — not
only the highlighted hunks. Unchanged code that should have changed is
in scope.

Trigger: `/haskell-constraint-evasion`, "review the Haskell diff",
"constraint evasion", "string stuffing".
